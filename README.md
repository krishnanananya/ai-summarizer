...
The Today tab is an inverted-pyramid daily brief for a 5–10 minute check-in: a Gemini-edited **lead story** (biggest visual weight), ranked **top stories 2–6** (each: headline, one "why it matters" line, source links, relative-traction meter, new-since-last-visit dot), then **Also on the radar** one-liners for breadth, with the per-kind tabs as the deep dive. Stories are generated at refresh time: Gemini gets traction-ranked candidates **plus every recent frontier-lab watchlist hit** (`lib/editorial.ts` — lab regexes + release-language detection force-include new releases that haven't accumulated traction yet) and an editorial rubric (frontier-lab releases > policy/safety > papers/benchmarks > everything else), and returns ranked stories referencing item ids, so one story bundles a launch article + its weights + its thread. If the digest call fails, Today falls back to a type-grouped tile grid. A **✓ Caught up** button advances the last-visit boundary on demand, and a bookmark on every card/row feeds a **Saved for later** list at the bottom of Today (snapshots in localStorage, so saved links outlive the feed windows).
...
- A daily cron hits `/api/refresh` (GET, `CRON_SECRET`-protected), which overwrites one cached blob (Upstash Redis in prod, `.cache/results.json` locally). The app reads only that blob, so loads are instant. If every source fails, the last good cache is kept.
- The header's refresh button POSTs to the same endpoint — no secret needed because it is debounced server-side (a no-op while the blob is under 30 minutes old) and lock-guarded against concurrent runs, so the worst an abuser gets is one pipeline run per half hour.
...
- After the pipeline, the refresh makes two fail-soft Gemini calls (needs `GEMINI_API_KEY`, same key as chat): one writes the ranked story digest into the cached blob, one backfills one-line summaries for title-only articles (HN gives no summary text). No key, a timeout, or a quota error just skips the enrichment for that run.
